...
module.exports = {
    ...
    .addUserOption(option => option.setName('user').setRequired(true).setDescription('User to look up')),
  async execute(interaction) {
    ...
    const clientStatus = target.presence?.clientStatus instanceof Object ? Object.keys(target.presence.clientStatus) : "offline";
    ...
    const sortedRoles = target.roles.cache.map(role => role).sort((a,b) => b.position - a.position).slice(0, target.roles.cache.size - 1)
    ...
    .addFields(
      ...
      {
        name: "Activities",
        value: target.presence?.activities.map(activity => `${activityType[activity.type]} ${activity.name}`).join('\n') || "None",
        inline: true,
      },
      {
        name: "Devices",
        value: devices.map(device => `${device.emoji} ${device.text}`).join(`\n`),
        inline: true,
      },

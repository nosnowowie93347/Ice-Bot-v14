...
module.exports = { 
    ...
    .addUserOption((option) =>
      option.setName("user").setRequired(false).setDescription("user to check"),
    ),
    async execute(interaction, client) {
        ...
        const user = await User.findOne({ guildId: interaction.guild.id, userId: mention });
        if (!user) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: "Coin:", value: `\`$${numberWithCommas(user.balance)}\``, inline: true })
            .addFields({ name: "Bank:", value: `\`$${numberWithCommas(user.bank)}\``, inline: true })
            .addFields({ name: "Total:", value: `\`$${numberWithCommas(user.balance + user.bank)}\``, inline: true })
            .setTimestamp();

...
module.exports = { 
    ...
    .addUserOption((option) =>
      option.setName("user").setRequired(true).setDescription("user to pay")
    ),
    async execute(interaction, client) {
        ...
        console.log(user)
        let target = await User.findOne({ guildId: interaction.guild.id, userId: member.id });
        if (!target) {
            target = new User({
                userId: member.id,
        ...
        if (args.toLowerCase() == 'all') { /// PAY ALL
            const embed = new EmbedBuilder()
                .setColor("Random")
                .setAuthor({ name: interaction.user.tag, iconURL: interaction.user.avatarURL({ dynamic: true }) })
                .setDescription(`You pay \`$${numberWithCommas(user.balance)}\` to ${member}.`)
                .setTimestamp();

...
module.exports = {
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) =>
                    option.setName("message").setDescription("Welcome Message"),
                )
                .addRoleOption((option) =>
                    option.setName("role").setDescription("Welcome Role"),
                )
                .addChannelOption((option) =>
                    option.setName("rule").setDescription("Rules Channel"),
                ),
        )
        .addSubcommand((subcommand) =>
            subcommand
    ...
    async execute(interaction, client) {
        if (interaction.options.getSubcommand() === "set") {
            ...
            if (data) {
                // If Welcome System Already Enabled
...
`);
            ...
            if (!data) {
                // If Setting Up Welcome System For First Time
                const channel = interaction.options.getChannel("channel");
                let message = interaction.options.getString("message");
                if (!message) message = null;
                let role = interaction.options.getRole("role");
        ...
        if (interaction.options.getSubcommand() === "remove") {
            ...
            if (!data) {
                await interaction.reply({
                    content: `Welcome System Not Setup In **${interaction.guild.name}**`,
                    ephemeral: true,
                });
            } else {
                ...
                await interaction.reply({
                    content: `Welcome System Disabled For **${interaction.guild.name}**`,
                });
            }
        }
    },
